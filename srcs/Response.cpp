...
{	
	...
	_target= concatenatePaths(_client->_config->root, _client->request.getPath(), "");		// combina el root del server con el path del request
	std::cout << Blue <<"target: " << _target << std::endl;
	
	if (_client->request.getMethod() == GET )			// si el método del request es GET o HEAD
    {
    ...
    if(_client->request.getMethod() == GET || _code != 200) 	// si el método del request no es HEAD y el método del request es GET o el código no es 200
    {
